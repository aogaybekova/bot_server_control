...
#мои модельки
def crash_process():
    target_containers = ['prod_antifraud', 'prod_nerez', 'prod_repeated', 'prod_crimea', 'prod_all']
    ...
    for line in lines:
        try:
            ...
            running_containers[name] = state
        except json.JSONDecodeError:
            continue

    crashed = []
    for name in target_containers:
        state = running_containers.get(name, 'not found')
        if state != 'running':
            crashed.append(pd.Series({'Name': name, 'State': state}))
